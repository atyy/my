...
<template>
    ...
    <gw-header></gw-header>
      ...
      <div class="reports-3-content" v-if="info">
        ...
        <!-- 模块消息-->
        <div class="solution">
          <collapses custom name="影响因素" :typeStyle="typeStyle">
            <div class="sub_wrap">
                ...
                <ul>
                    <li>
                      <div class="para_title">
                        <i class="icon reports_5"></i>
                        <label class="name" v-html="info.report['CN'].factors.genetic.name"></label>
                      </div>
                      <p class="about" v-html="info.report['CN'].factors.genetic.about"></p>
                    </li>
                </ul>
              </div>
          ...
          <collapses custom name="临床表现" :typeStyle="typeStyle" v-if="info.report['CN'].symptom">
            <div class="sub_wrap">
              ...
              <div class="txt" v-html="info.report['CN'].symptom"></div>
          ...
          <collapses custom name="食物护理" :typeStyle="typeStyle" v-if="info.report['CN'].food">
            <div class="sub_wrap">
              ...
              <div class="txt" v-html="info.report['CN'].food"></div>
      ...
      this.$axios({
        ...
        if(isSuccess){
          ...
          if(data.phenotype.length > 0){
            ...
            //  latex独立处理
            if(self.info.report['CN'].factors.genetic.about){
              self.info.report['CN'].factors.genetic.about = self.customLatex(self.info.report['CN'].factors.genetic.about);
            }
          } else {
            self.$dialog.alert({
              mes: '您没有该表型结果',
